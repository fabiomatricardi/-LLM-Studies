...
### Trying to run embeddings over llamafile
llama-cpp-compatible API server
- [ ] ISSUE: I cannot get any similarity search
- to be investicgated
```
...
# WITH FAISS  https://python.langchain.com/v0.2/docs/integrations/vectorstores/faiss/
# WITH CHROMA https://python.langchain.com/v0.2/docs/integrations/vectorstores/chroma/
###############################################################################
...
console.print('Splitting documents')
loader = TextLoader("china.txt", encoding='utf-8')
documents = loader.load()
text_splitter = TokenTextSplitter(chunk_size=200, chunk_overlap=50)
texts = text_splitter.split_documents(documents)
#create the vector store
console.print('Creating DB')
db = FAISS.from_documents(texts, embeddings)
console.print(f'Lenght docs = {len(texts)}')
console.print(texts)
console.print(db)
console.print('Db completed')
                         
query = 'high-tech'

console.print('[bold red1]Testing one similarity search retriever...')
#default is similarity search
console.print('embedding query')
query_result = embeddings.embed_query(query)
docs = db.similarity_search(query,k=2)  # original (query,k=2)
console.print(docs)
console.print('---')
console.print(f'Lenght docs = {len(texts)}')
...
### how to get model name from OpenAI client connection
running a llama-cpp-server OpenAI API compatible from [source](https://huggingface.co/internlm/internlm2_5-7b-chat-gguf#serving)
